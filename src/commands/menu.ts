...
import { infoChannelId, infoMessageId } from "../config.json";
...
const menu: {
  ...
  actions: async (inter: ButtonInteraction | StringSelectMenuInteraction) => {
    switch (inter.customId) {
      ...
      // Roles > District
      case "selectDistrict":
        if (inter.isStringSelectMenu()) return await giveRole(inter, inter.values[0]);
      // Roles > Age
      case "<15":
  ...
  feedback: {
    modal: new ModalBuilder()
      ...
      .setTitle("Отзыв")
      .addComponents(
        new ActionRowBuilder<TextInputBuilder>().addComponents(new TextInputBuilder().setCustomId("feedbackInput").setLabel("Здесь ты можешь оставить свой отзыв:").setStyle(TextInputStyle.Paragraph))
      ),
  ...
  messages: {
    ...
    guide: {
      ...
      components: [
        new ActionRowBuilder<ButtonBuilder>().addComponents([new ButtonBuilder().setLabel("Начать").setCustomId("guide1").setStyle(ButtonStyle.Success).setEmoji("<:play:1042704531835867217>")]),
      ],
          ...
          new EmbedBuilder()
            ...
            .setDescription(
              "<:shield:1036567108106072084> <@&1034415439910019113>\n" +
                ...
                "	Главный бот сервера, <@1042400695489400884>. Именно с помощью этого бота ты читаешь это руководство.\n\n"
            ),
        ],
        components: [
      ...
      secondPage: {
        embeds: [
          new EmbedBuilder()
            ...
            .setDescription(
              "**⌛ События**\n" +
                "В этой категории можно найти информацию о предстоящих и прошедших событиях, а так же их обсуждения. \n\n" +
                "**💡 Саморазвитие**\n" +
                "Тут ты можешь начать обсуждение на какую-то конкретную тему и поделиться своим мнением. Дополнительную информацию о форумах этой категории можно узнать на следующей странице руководства. \n\n" +
                "**💬 Чат**\n" +
                'Категория "Чат" содержит в себе... правильно, чаты. \n\n' +
                "**🏳 Округа**\n" +
                'В чатах этой категории можно общаться с участниками сервера из твоего города/области, а так же планировать встречи. Чтобы получить доступ к этим чатам, нужно взять роль в разделе "Роли" главного меню. \n\n' +
                "**🎙 Аудио**\n" +
                'То же самое, что и категория "Чат", только с голосовыми чатами. \n\n' +
                ""
            ),
        ],
        components: [
      ...
      thirdPage: {
        embeds: [
          new EmbedBuilder()
            ...
            .setColor(0x2f3136)
            .setDescription(
              "Форумы - специальные каналы для разнообразных обсуждений. У каждого форума на сервере есть своя тема (в названии) и теги, которыми можно точнее обозначить тему cвоего поста. Ниже перечислены несколько возможных тем для каждого форума:\n\n" +
                "<#1034420252295057439> — боевые искусства, спорт, тренировки\n" +
                "<#1034420491701719071> — инвестиции, предпринимательство, финансы\n" +
                "<#1034420217679454209> — психическое здоровье, медитация, зависимости\n" +
        ...
        new EmbedBuilder()
          ...
          .setDescription(
            "Чтобы настроить уведомления, получить возможность участвовать во встречах, а так же помочь другим понять тебя и твою жизненную ситуацию, " +
              "выбери одну из категорий ниже и возьми соответствующие роли."
          ),
      ],
      components: [
        new ActionRowBuilder<ButtonBuilder>().addComponents([
      ...
      notifications: {
        ...
        components: [
          new ActionRowBuilder<ButtonBuilder>().addComponents([
            ...
            new ButtonBuilder().setLabel("Международные События").setCustomId("international").setStyle(ButtonStyle.Primary).setEmoji("<:globe:1043623333306056725>"),
          ]),
          new ActionRowBuilder<ButtonBuilder>().addComponents([
            new ButtonBuilder().setLabel("Назад").setCustomId("backToRoles").setStyle(ButtonStyle.Secondary).setEmoji("<:angleleftb:1042704516241444904>"),
            new ButtonBuilder().setLabel("Adonis.House").setStyle(ButtonStyle.Link).setEmoji("<:adonishouse:1043624032555241514>").setURL("https://discord.gg/mWzZBNwqz6"),
          ]),
        ],
      },
    },
